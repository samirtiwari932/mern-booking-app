...
const Page = () => {
    ...
    return (
        <form className='flex flex-col gap-5' onSubmit={onSubmit}>
            ...
            <label className='text-gray-700 text-sm font-bold flex-1'>
                Confirm Password
                <input type='password' autoComplete="on" className='border rounded w-full py-1 px-2 font-normal' {...register("confirmPassword", {
                    validate: (val) => {
                        if (!val) {
            ...
            <span className='flex items-center justify-between' >
                <span className='text-sm'>
                    ...
                    <Link href='/sign-in' className='underline hover:text-blue-600'>Login </Link>
                </span>
                <button
                    type='submit'
